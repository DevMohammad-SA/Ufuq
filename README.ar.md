...
> ⚠️ **ملاحظة تاريخية:** نسخة سابقة من هذا الملف كانت تصف دخول المشارك بأنه
> "بلا كلمة مرور". **هذا التصميم أُلغي نهائيًا** — المشارك الآن يدخل بكلمة
> مرور حقيقية. راجع [`docs/authentication.md`](docs/authentication.md).
> كذلك يوثّق `docs/known-limitations.md` في قسم مخصص عدة أمور أخرى كانت
> موصوفة هنا سابقًا وتغيّرت منذ ذلك الحين (بناء إعداد Docker/PostgreSQL
> إنتاجي، وجود ملف ترويسة PDF الآن، تحوّل `Group.supervisor` من علاقة
> واحد-لمتعدد إلى متعدد-لمتعدد، وإمكانية منح 10 أو 12 نقطة عند قبول مهمة) —
> يُنصح بقراءته قبل افتراض أن وصف هذا الملف الحالي حديث بالكامل هو الآخر.
...
| الدور | القيمة | ملخص |
|------|--------|------|
| مشارك | `participant` | يدخل بـ **رقم الهوية + كلمة المرور**. لوحة شخصية (فيها سجل نقاط مدمج ومؤشر رحلة النخبة)، رفع المهمة الأسبوعية، المتجر. |
...
| مشرف عام | `general_supervisor` | على مستوى البرنامج: استيراد إكسل، استمارة إضافة مشارك مفرد، المهام الأسبوعية، إدارة المتجر، منح نقاط إضافية يدويًا، تصفير النقاط، سجل النقاط الكامل، كل البيئات. |
